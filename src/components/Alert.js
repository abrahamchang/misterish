...
const Alert = ({ title, text, onPressOk, onPressCancel }) => {
    ...
    return (
        <View style={viewStyle}>
            <Text style={{ color: '#36175E', fontWeight: 'bold', marginTop: '3%'  }}>{title}</Text>
            <View style={textArea}>
                <Text style={{ color: 'white', alignItems: 'center', justifyContent: 'center' }}>{text}</Text>
            </View>
...
const styles = {
    viewStyle: {
        width: '80%',
        height: '60%',
        backgroundColor: 'rgba(85, 50, 133, 0.5)',
        elevation: 2,
